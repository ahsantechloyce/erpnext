...
class Timesheet(Document):
	...
	def validate(self):
		print "before employee name"
		self.set_employee_name()
		print "before set status "
		self.set_status()
		print "before validate dates "
		self.validate_dates()
		print "before time logs"
		self.validate_time_logs()
		print "before update cost"
		self.update_cost()
		print "before total amount calculate"
		self.calculate_total_amounts()
		print "before calculate percentage billed"
		self.calculate_percentage_billed()
		print "before set dates"
		self.set_dates()
	...
	def validate_mandatory_fields(self):
		...
		for data in self.time_logs:
			...
			if not self.employee:
				frappe.throw(_("Row {0}: Employee is mandatory.").format(data.idx))
	...
	def validate_time_logs(self):
		print "Before validate time log"
		for data in self.get('time_logs'):
			self.check_workstation_timings(data)
			self.validate_overlap(data)
		print "After validate time log"
	...
	def update_cost(self):
		for data in self.time_logs:
			...
			# 	print rate,"Activity Rates AREEEEE"
			...
			if data.billable:
				...
				data.billing_amount = data.billing_rate * hours
				data.costing_amount = data.costing_rate * hours
				print data.billing_amount,data.costing_amount,data.billing_rate,data.costing_rate
...
@frappe.whitelist()
def get_emplooyee_cost_values(employee=None):
	rate = frappe.db.get_values("Employee", {"name": employee}, ["costing_rate", "billing_rate"], as_dict=True)
	return rate[0] if rate else {}
